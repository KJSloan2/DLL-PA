...
gridStatsGeoJson_path = os.path.join(parent_dir, "data", "composite", "gridStats.geojson")
placeRefGeoJson_path = os.path.join(parent_dir, "resources", "reference", "tl_2024_"+selectedStateCode+"_place.geojson")
#acsSummary_path: acs data aggragated and summarized by USCB place
acsSummaryGeoJson_path = os.path.join(parent_dir, "data", "acs", "acsCompositeStats_uscbPlace.geojson")
hcGridRefGeoJson_path = os.path.join(parent_dir, "data", "healthcare", "healthcareFacilities_gridStats.geojson")
######################################################################################
...
with open(placeRefGeoJson_path, "r") as placeRefGeoJson_file:
    placeRefGeoJson = json.load(placeRefGeoJson_file)

with open(hcGridRefGeoJson_path, "r") as hcGridRefGeoJson_file:
    hcGridRefGeoJson = json.load(hcGridRefGeoJson_file)
######################################################################################
# Read gridStatsGeoJson and create a reference list of feature ids for easy reference
with open(gridStatsGeoJson_path, "r") as gridStatsGeoJson_file:
    gridStatsGeoJson = json.load(gridStatsGeoJson_file)

refIds_gridStats = []
for feature in gridStatsGeoJson["features"]:
    feature_id = feature["properties"]["id"]
    refIds_gridStats.append(feature_id)
######################################################################################
# Healchare Facilities
# Read hcGridStatsGeoJson and create a reference list of feature ids for easy reference
with open(hcGridRefGeoJson_path, "r") as hcGridStatsGeoJson_file:
    hcGridStatsGeoJson = json.load(hcGridStatsGeoJson_file)

refIds_hcGridStats = []
for feature in hcGridStatsGeoJson["features"]:
    feature_id = feature["properties"]["id"]
    print(feature_id)
    refIds_hcGridStats.append(feature_id)
######################################################################################

with open(acsSummaryGeoJson_path, "r") as acsSummary_file:
...
extremeWeatherPropsToGet = [
    #Wind
    "wa_mag_mean_t", "wa_inj_mean_t", "wa_fat_mean_t", 
    "wa_loss_mean_t", "wa_closs_mean_t", "wa_len_mean_t", "wa_wid_mean_t",
    "wa_inj_sum", "wa_fat_sum", "wa_loss_sum", "wa_closs_sum",
    #Tornados
...
acsPropsToGet = [
    "s0101_c01_001e", "dp03_0001e", "dp03_0022e", "dp03_0023e",
    "dp03_0024e", "dp03_0025e", "dp03_0032e", "dp03_0033e",
    "dp03_0034e", "dp03_0035e", "dp03_0036e", "dp03_0037e",
    "dp03_0038e", "dp03_0039e", "dp03_0040e", "dp03_0041e",
...
count = 0
overlaps_found = 0
# gridRefGeoJson: state-filtered grid cells
for gridFeature in gridRefGeoJson["features"]:
    try:
        gridGeom = shape(gridFeature["geometry"])
        grid_id = gridFeature["properties"]["id"]
        updatedGridFeature = make_new_geojson_feature(
            gridFeature, newProperties={"uscb_geoid": [], "prct_overlap": []})
        ...
        if ewGridStatsFeature_idx is not None:
            ew_props = gridStatsGeoJson["features"][ewGridStatsFeature_idx]["properties"]
            for propKey in extremeWeatherPropsToGet:
                if propKey in ew_props.keys():
                    updatedGridFeature["properties"][propKey] = ew_props[propKey]

        hcGridStatsFeature_idx = refIds_hcGridStats.index(grid_id) if grid_id in refIds_hcGridStats else None
        #print(hcGridStatsFeature_idx)
        if hcGridStatsFeature_idx is not None:
            hc_props = hcGridStatsGeoJson["features"][hcGridStatsFeature_idx]["properties"]
            temp_hcProps = []
            for propKey in hcPropsToGet:
                #print(propKey)
                if propKey in hc_props.keys():
                    updatedGridFeature["properties"][propKey] = hc_props[propKey]
                    temp_hcProps.append(hc_props[propKey])
            print(temp_hcProps)
        ##########################################################################
        '''Process intersections and overlaps of USCB places with grid cells.
        Add ACS stats to grid cells with USCB place overlaps.'''             
        for placeFeature in placeRefGeoJson["features"]:
            try:
...
output_path = os.path.join(public_dir, "gridStats_"+selectedStateCode+".geojson")
