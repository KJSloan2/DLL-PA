...
row_sizes = [csv_row_size_bytes(row) for row in rows]
...
if rowCount > rowsPerFile:
    ...
    if "csv" in exportAs:
        ...
        write_to_csv(output_path, headers, rows)
    if "geojson" in exportAs:
        output_path = os.path.join(mapping["output_dir"], output_fName+".geojson")
        write_to_geojson(output_path, headers, rows)
