...
print("aoi_bb_pt_sw: ", aoi_bb_pt_sw)
print("aoi_bb_pt_ne: ", aoi_bb_pt_ne)
...
print("raster_bb_pt_sw: ", raster_bb_pt_sw)
print("raster_bb_pt_ne: ", raster_bb_pt_ne)

def compute_pixel_offset(target_bb_pt, raster_bb_pt_sw, raster_bb_pt_ne):
    """
    Compute pixel offset accounting for inverted y-axis in raster data.
    Raster origin is at top-left (NW), but we calculate from bottom-left (SW).
    """
    dlon = target_bb_pt[0] - raster_bb_pt_sw[0]
    # For latitude, calculate from the top (north) edge
    dlat = raster_bb_pt_ne[1] - target_bb_pt[1]
    
    lat_ref = math.radians((target_bb_pt[1] + raster_bb_pt_sw[1]) / 2.0)
...
targetIdxs_start = compute_pixel_offset(aoi_bb_pt_sw, raster_bb_pt_sw, raster_bb_pt_ne)
targetIdxs_end = compute_pixel_offset(aoi_bb_pt_ne, raster_bb_pt_sw, raster_bb_pt_ne)
...
band_window = band[
    targetIdxs_end[1]:targetIdxs_start[1],
    targetIdxs_start[0]:targetIdxs_end[0]
]

'''# Switched to using rasterio windows for better accuracy
window = from_bounds(
    aoi_bb_pt_sw[0], aoi_bb_pt_sw[1],  # left, bottom
    aoi_bb_pt_ne[0], aoi_bb_pt_ne[1],  # right, top
    src.transform
)
band_window = src.read(1, window=window)'''
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - #
